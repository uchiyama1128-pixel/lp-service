...
def build_lp_html(hearing: dict, copy: dict, embed_images: bool = False) -> str:
    ...
    for item in hearing.get("main_menu", []):
        ...
        if _photo_path:
            ...
            _mphoto_html = '<div class="menu-card-img-wrap menu-card-img-blank"></div>'
...
<head>
  ...
  <style>
    ...
    *  {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .intro-target {{ background: color-mix(in srgb, var(--p) 80%, #fff); padding: 0; display: flex; flex-direction: row; border: 1px solid rgba(255,255,255,.2); box-shadow: 0 4px 20px rgba(0,0,0,0.12); overflow: hidden; }}
    .intro-target-body {{ padding: 28px 32px; display: flex; flex-direction: column; justify-content: center; flex: 1; }}
    ...
    .menu-card-img-wrap {{ width: 100%; aspect-ratio: 4/3; overflow: hidden; }}
    .menu-card-img {{ width: 100%; height: 120%; object-fit: cover; object-position: center top; display: block; }}
    .menu-card-img-blank {{ background: linear-gradient(135deg,var(--cream),#e0d5c8); }}
